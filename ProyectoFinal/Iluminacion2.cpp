...
int main()
{
	...
	Model logobumpadel((char*)"Models/bumpersadelante/logobump.obj");
	Model basebumpadel((char*)"Models/bumpersadelante/basebump.obj");
	Model logobumpatras((char*)"Models/bumperatras/logobumper.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//logo bumper adelante
		model = glm::mat4(1);
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glUniform4f(glGetUniformLocation(lightingShader.Program, "colorAlpha"), 1.0f, 1.0f, 1.0f, 1.0f);
		logobumpadel.Draw(lightingShader);
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_Z] && rot < 75.0f) // Si se presiona la tecla Z y la rotación es menor a 75 grados.
	{
		rot += 100.0f * deltaTime; // Incrementa la rotación.
		if (rot > 75.0f) // Limita la rotación para que no exceda los 75 grados.
		{
			rot = 75.0f;
		}
	}
	else if (!keys[GLFW_KEY_Z] && rot > 0.0f) // Si se suelta la tecla Z y la rotación es mayor a 0 grados.
	{
		rot -= 100.0f * deltaTime; // Decrementa la rotación.
		if (rot < 0.0f) // Limita la rotación para que no sea menor a 0 grados.
		{
			rot = 0.0f;
		}
	}
