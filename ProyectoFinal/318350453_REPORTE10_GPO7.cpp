...
glm::vec3 pointLightPositions[] = {
	glm::vec3(-3.0f,0.0f, -3.0f), //PointLight1
	glm::vec3(-3.0f,0.0f, 3.0f), //PointLight2
	glm::vec3(3.0f,0.0f,  3.0f), //PointLight3
	glm::vec3(3.0f,0.0f, -3.0f) //PointLight4
};
...
int main()
{
	...
	Model Falda((char*)"Models/Bumper/Falda.obj");
	Model Bumper((char*)"Models/Bumper/Bumper.obj");
	Model Palanca((char*)"Models/Lever/Palanca.obj");
	Model Cuerpo((char*)"Models/Clock/Body.obj");
	Model Reloj((char*)"Models/Clock/Clock.obj");
	Model Canica((char*)"Models/Canica/Canica.obj");
	Model Palanca2((char*)"Models/Palanca2/Lever2.obj");
	Model Pinball((char*)"Models/Pinball/Mesa.obj");
	Model Busto((char*)"Models/Spot/spot.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[1].linear"), 0.6f);
		...
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[2].linear"), 0.45f);
		...
		glUniform1f(glGetUniformLocation(lightingShader.Program, "pointLights[3].linear"), 0.45f);
		...
		//Falda.Draw(lightingShader);
		//Bumper.Draw(lightingShader);
		...
		//glUniform1f(glGetUniformLocation(lightingShader.Program, "material.shininess"), 90.0f);
		//glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void DoMovement()
{
	...
	if (keys[GLFW_KEY_U])
	{
		spotlight.Pos.z -= 0.1f;
	}
	if (keys[GLFW_KEY_J])
	{
		spotlight.Pos.z += 0.01f;
	}

	if (keys[GLFW_KEY_Z])
	{
		spotlight.Dir.x += 0.01f;
	}if (keys[GLFW_KEY_X])
	{
		spotlight.Dir.x -= 0.01f;
	}if (keys[GLFW_KEY_C])
	{
		spotlight.Dir.y += 0.01f;
	}if (keys[GLFW_KEY_V])
	{
		spotlight.Dir.y -= 0.01f;
	}if (keys[GLFW_KEY_B])
	{
		spotlight.Dir.z += 0.01f;
	}if (keys[GLFW_KEY_N])
	{
		spotlight.Dir.z -= 0.01f;
	}
